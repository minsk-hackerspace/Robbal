...
Для того чтобы настроить модуль, он подсоединяется через любомй USB-UART переходник к компьютеру с Ardiono IDE. Важно джамперои или переключателем выставить напряжение UART 3.3 вольта.
