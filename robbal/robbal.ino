...
int acc_calibration_value = -660;                            //Enter the accelerometer calibration value
...
float max_fwd_speed = 6;
float max_back_speed = 4;                               
...
void setup(){
  ...
  Serial.println("hello balancing robot");
  ...
  pinMode(2, OUTPUT); //pulse                                                      //Configure digital poort 2 as output
  pinMode(3, OUTPUT); //dir                                                      //Configure digital poort 3 as output
  pinMode(4, OUTPUT); //pulse                                                      //Configure digital poort 4 as output
  pinMode(5, OUTPUT); //dir                                                      //Configure digital poort 5 as output
  ...
  for(receive_counter = 0; receive_counter < 500; receive_counter++){       //Create 500 loops
    ...
    delayMicroseconds(5000); //Wait for 5000 microseconds to simulate the main program loop time
    if(receive_counter % 30 == 0) {
      Serial.println(".");
    }
...
void loop(){
  ...
  if(char(received_byte) == '^'){    
    ...
    if(pid_setpoint > -max_fwd_speed) pid_setpoint -= 0.05;                            //Slowly change the setpoint angle so the robot starts leaning forewards
  ...
  if(char(received_byte) == '_'){ 
    Serial.println("_");
    //If the forth bit of the receive byte is set change the left and right variable to turn the robot to the right
    if(pid_setpoint < max_back_speed)pid_setpoint += 0.05;                             //Slowly change the setpoint angle so the robot starts leaning backwards
  ...
  if(char(received_byte) == '('){ 
    ...
    pid_output_left += turning_speed;
    pid_output_right -= turning_speed;
    if(pid_setpoint < max_back_speed) pid_setpoint += 0.05;
  ...
  if(char(received_byte) == '{'){ 
    ...
    pid_output_left += turning_speed;
    pid_output_right -= turning_speed;
    if(pid_setpoint > -max_fwd_speed) pid_setpoint -= 0.05;
  ...
  if(slowdown){                                         //Slowly reduce the setpoint to zero if no foreward or backward command is given
